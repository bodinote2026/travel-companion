...
function resolveInitialRegions(codes?: string[] | null): string[] {

  const normalized = normalizeUserRegions(codes ?? []);
...
export function ProfileSetupForm({
  ...
  const [regions, setRegions] = useState<string[]>(resolveInitialRegions(initialRegions));
  ...
  function toggleRegion(code: string) {
    ...
    if (!isKnownRegionCode(code)) return;
    ...
    setRegions((prev) => {
      ...
      return normalizeUserRegions([...prev, code]);
  ...
  function validateRequiredIdentity(): string | null {
    ...
    if (regions.length === 0) return '활동 지역을 하나 이상 선택해주세요.';

    if (regions.some((code) => !isKnownRegionCode(code))) {
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <label className="block">
          ...
          <span className="mt-1 block text-xs text-muted-foreground">
            ...
            숫자만 10~11자리로 입력해 주세요. 모집 확정/구매 확정을 위해 정확히 입력해주세요.
        ...
        <fieldset>
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">중복 선택 가능</p>

          <div className="mt-2 flex flex-wrap gap-2">

            {REGION_OPTIONS.map((option) => {

              const selected = regions.includes(option.code);
